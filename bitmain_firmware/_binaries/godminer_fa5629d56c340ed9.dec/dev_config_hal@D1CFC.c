int __fastcall dev_config_hal(char *a1)
{
  ...
  if ( (int)a1 <= 3000000 )
  {
    ...
    if ( (int)a1 > 921600 )
    {
      if ( a1 == "t cnt: %08x" || a1 == byte_17D784 )
      {
        v2 = 16843009;
        v3 = 1;
        goto LABEL_13;
...
LABEL_13:
  LOWORD(v4) = -25516;
  LOWORD(v5) = -25340;
  HIWORD(v4) = (unsigned int)"check fail. store_crc = 0x%x, len = 0x%x, crc = 0x%x\n" >> 16;
  HIWORD(v5) = (unsigned int)"rom_encryption_algorithm error! eeprom_key_version:%d, eeprom_encryption_algorithm:%d" >> 16;
  printf(v5, v4, v3);
  v7 = v2;
