int __fastcall set_json_info_by_errorcode_and_time(_DWORD *a1, char *a2, time_t a3)
{
  ...
  if ( v6 || (result = miner_6060info_errorcode_init()) != 0 )
  {
    ...
    if ( result )
    {
      ...
      v9 = BUFX_strdup(a2);
      LOWORD(v10) = 31592;
      HIWORD(v10) = (unsigned int)&loc_146898 >> 16;
      json_object_set_new(v8, v10, v9);
      v11 = BUFX_strdup(v20);
      LOWORD(v12) = -26380;
      HIWORD(v12) = (unsigned int)"_voltage failed!, pls check <<" >> 16;
      json_object_set_new(v8, v12, v11);
      v13 = BUFX_strdup(v21);
      LOWORD(v14) = -26448;
      HIWORD(v14) = (unsigned int)"chdog: 0x%04x" >> 16;
      json_object_set_new(v8, v14, v13);
      localtime_r(&timer, &tp);
      LOWORD(v15) = -26372;
      HIWORD(v15) = (unsigned int)" failed!, pls check <<" >> 16;
      snprintf(s, 0x100u, v15, tp.tm_year + 1900, tp.tm_mon + 1, tp.tm_mday, tp.tm_hour, tp.tm_min, tp.tm_sec);
      v16 = BUFX_strdup(s);
      LOWORD(v17) = -26348;
      HIWORD(v17) = (unsigned int)">> chain avg vol %s from %d to %.2f, pls check <<" >> 16;
